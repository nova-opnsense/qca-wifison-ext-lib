...
#define RECV_BIT_SET(bitmap, bit) ((bitmap) |= (1 << (bit)))
#define RECV_BIT_IS_SET(bitmap, bit) (((bitmap) & (1 << (bit))) != 0)
...
static int recv_events(struct nl_msg *msg, void *arg)
{
	...
	printf("[%s:%d] (%s): \r\n", __FILE__, __LINE__, __func__);
	if (RECV_BIT_IS_SET(ctx.recv_flags, report->type))
	{
		fprintf(stdout, "\nReport Type = %u\n", report->type);
...
int main(int argc, char *argv[])
{
	...
	do
	{
		...
		case 'h': /* -h or --help */
			print_usage(stdout, 0);
		case 'a': /* -a or --assoc */
			RECV_BIT_SET(ctx.recv_flags, ASSOC_STA_REPORT);
			break;
		case 'n': /* -n or --nonassoc */
			RECV_BIT_SET(ctx.recv_flags, NON_ASSOC_STA_REPORT);
			break;
		case 'v': /* -v or --verbose */
			verbose = 1;
			break;
